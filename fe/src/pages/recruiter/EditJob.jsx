...
const EditJob = () => {
  ...
  const fetchJobData = async () => {
    try {
      ...
      const response = await recruiterService.getJobById(id);
      
      if (response.success && response.data) {
        const job = response.data;
        
        // Format application deadline for input[type="date"]
        ...
        setFormData({
          ...
          location: {
            ...
            country: job.location?.country || 'Vietnam'
          },
          experience_required: {
            min: job.experience_required?.min || 0,
            max: job.experience_required?.max || ''
          },
          education_required: job.education_required || 'not_required',
          skills_required: job.skills_required || [],
          application_deadline: applicationDeadline,
          category_id: job.category_id?._id || job.category_id || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await recruiterService.updateJob(id, submitData);
      
      if (response.success) {
        toast.success('Cập nhật công việc thành công!');
        navigate('/recruiter/jobs');
      } else {
        throw new Error(response.message || 'Có lỗi xảy ra khi cập nhật công việc');
      }
    } catch (error) {
      console.error('Update job error:', error);
      const errorMessage = error.response?.data?.message || error.message || 'Không thể cập nhật công việc';
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="mb-8">
        <div className="flex items-center space-x-3 mb-4">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Chỉnh sửa công việc</h1>
        </div>
        <p className="text-gray-600">Cập nhật thông tin chi tiết về vị trí tuyển dụng</p>
      </div>
